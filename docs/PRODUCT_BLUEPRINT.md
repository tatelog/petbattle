...
Lv.1とLv.2 SVG Questが現在の実装値であり、Lv.3以降はロードマップ上の設計目標である。容量は受け付け可能な入力サイズであり、能力値ではない。エッセンス上限は表現の選択肢を増やすCore容量である。

| Core Level | 状態 | 主な形式 | 入力容量の目安 | エッセンス上限 | 学ぶ内容 |
|---|---|---|---:|---:|---|
| Lv.1 | 実装済み | JPEG / PNG / WebP | 2 MiB | 16 | 画像、色、構図、AIへの指示と意味認識 |
| Lv.2 | 実装済み | SVG | 5 MiB | 32 | 座標、パス、レイヤー、ベクター表現 |
| Lv.3 | 設計目標 | JSON Effect Recipe、コード生成物 | 10 MiB | 64 | 関数、反復、状態、デバッグ |
| Lv.4 | 設計目標 | GLB / OBJ / STL | 25 MiB | 128 | 頂点、面、法線、素材、3Dプリント |
| Lv.5 | 設計目標 | PDF / IFC | 50 MiB | 256 | 部品階層、属性、関係、BIMの意味構造 |
...
現在のLv.2 QuestはAPI不要のルールベースメンターとして、テーマごとの観察・分解・構築・検証を実際に提示する。将来のAIメンターも答えを一括生成するのではなく、次を担当する。
...
教育カテゴリへ訴求するため、「学ぶ → 作る → 検証される → PETが変わる」をLv.2 SVG Questとして実装した。テーマと焦点を選び、元画像を参照しないSVGを生成し、制作理由と成果物をポートフォリオへ保存して再戦できる。
