...
開始演出は4.8秒の`easeInOutCubic`で、カメラがコロシアム上空を約189度旋回しながら、半径と高度を下げて正面構図へ入る。終盤には左右のPETが場外側から召喚台へスライドし、定位置へ並んだ後に`BATTLE START`を表示する。
...
| 2.5〜4.2秒 | 左右のPETを場外側から召喚台へスライドし、縮小状態から等倍へ整列 |
